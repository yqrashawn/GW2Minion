...
function devmonitor.ModuleInit()
	...
	GUI_NewField("devmonitor","InteractType","GadgetinteractType","GadgetInfo")	
	GUI_NewField("devmonitor","Name","Gadgetname","GadgetInfo")
	...
	GUI_NewField("devmonitor","Marker ID","MarkerEventID","MarkerInfo")
	GUI_NewField("devmonitor","Subregion ID","SubregionID","MarkerInfo")	
...
function devmonitor.UpdateWindow()
	...
	if (TID  ~= nil) and (TID ~= 0) then
		...
		if (mytarget ~= nil) then
			...
			if (mytarget ~= nil) then
				...
				GadgetinteractType = tostring(mytarget.contentID2)
				Gadgetname = mytarget.name or ""
	...
	if (mlist ~= nil) then	
		...
		if (i~=nil and event~=nil) then			
			...
			SubregionID = tostring(event.subregionID)			
