...
function c_gcombat_over:evaluate()
	...
	if ( wt_core_state_gcombat.CurrentTarget == nil or wt_core_state_gcombat.CurrentTarget == 0 ) then
		...
		if ( T == nil or not T.alive or not T.onmesh or T.attitude == 0 or T.attitude == 3) then
			if (wt_global_information.TargetIgnorelist ~= nil and wt_global_information.TargetIgnorelist[T.contentID] ~= nil and wt_global_information.TargetIgnorelist[T.contentID] < T.health.percent) then
				Player:ClearTarget()
				wt_core_state_gcombat.StopCM()
				return true
			elseif(wt_global_information.TargetBlacklist ~= nil and wt_global_information.TargetBlacklist[wt_core_state_gcombat.CurrentTarget] ~= nil ) then
				Player:ClearTarget()
				wt_core_state_gcombat.StopCM()
				return true
			end
