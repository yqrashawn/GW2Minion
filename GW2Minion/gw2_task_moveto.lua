...
function gw2_task_moveto:Process()
	...
	if ( ValidTable(ml_task_hub:CurrentTask().targetPos) ) then
		...
		if ( dist <= stopDistance) then
			...
			-- Waypoint Usage
			if (ml_task_hub:CurrentTask().useWaypoint == true and ml_global_information.Player_InCombat == false and Player.healthstate == GW2.HEALTHSTATE.Alive and dist > 15000 and Inventory:GetInventoryMoney() > 500) then
