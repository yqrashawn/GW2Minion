...
function profilePrototype:SwapWeapon( targetdist )
	...
	if ( Player.inCombat and ( Player:CanSwapWeaponSet() or ml_global_information.Player_CurrentWeaponSet == 2) ) then 
		if ( ml_global_information.Player_SwimState == GW2.SWIMSTATE.NotInWater ) then
			...
			if ( ml_global_information.Player_Profession ~= GW2.CHARCLASS.Engineer and ml_global_information.Player_Profession ~= GW2.CHARCLASS.Elementalist ) then -- this is needed, else the not existing weapon 2 is being added as "possible to swap to" set
