		...
		if (gw2_skill_manager.skillWindow.visible) then
			...
			if (GUI:BeginChild("##gw2sm-skillinfo", 0, 0, true)) then -- BUG: No border on child causes small layout glitch with scrollbars.
				...
				if (GUI:CollapsingHeader(GetString("player"))) then
					...
					GUI:NextColumn()
					GUI:PushItemWidth(GUI:GetContentRegionAvailWidth())
					currentSkill.player.combatState = gw2_skill_manager:combo("##gw2sm-combatState",currentSkill.player.combatState,gw2_skill_manager.combatState)
...
function profilePrototype:DoCombatMovement(targetID)
	...
	if (Settings.GW2Minion.combatmovement and table.valid(target) and self.tmp.combatMovement.allowed and not self.tmp.combatMovement.range and target.distance <= (self.tmp.activeSkillRange + 250) and noStopMovementBuffs and ml_global_information.Player_OnMesh and ml_global_information.Player_Alive and ml_global_information.Player_Health.percent < 99) then
