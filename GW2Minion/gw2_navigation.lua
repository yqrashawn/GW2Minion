...
function Player:MoveTo(x, y, z, targetid, stoppingdistance, randommovement, smoothturns)
	...
	ml_navigation.randommovement = false and randommovement
	ml_navigation.smoothturns = false and smoothturns or true
		...
		if ( GetGameState() == GW2.GAMESTATE.GAMEPLAY) then
			...
			if ( pathsize > 0 ) then
				if ( ml_navigation.pathindex <= pathsize ) then
					...
					if( ml_navigation.navconnection ) then
						...
						elseif(ml_navigation.navconnection.type == 4) then
							...
							if(ml_navigation.navconnection.subtype == 1 or ml_navigation.navconnection.subtype == 2 or ml_navigation.navconnection.subtype == 3) then
                nextnode = ml_navigation.path[ ml_navigation.pathindex + 1]
                ...
                return
							...
							if(ml_navigation.navconnection.subtype == 4 ) then
								-- INTERACT
								Player:Interact()
