...
function JSONTEST()
	...
	if ( table.valid(c) ) then 								
		...
		while (id and e) do
			...
			n["position"]["hy"] = nil
			n["position"]["hz"] = nil
			n["position"]["x"] = math.round(n["position"]["x"],2)
			n["position"]["y"] = math.round(n["position"]["y"],2)
			n["position"]["z"] = math.round(n["position"]["z"],2)
			n["position"]["mapid"] = Player:GetLocalMapID()
			n["maxhealth"] = e.health.max
			n["attitude"] = e.attitude			
			n["profession"] = e.profession
			n["level"] = e.level
			n["attackable"] = e.attackable
			n["isplayer"] = e.isplayer
			n["ispet"] = e.ispet
			n["ismonster"] = e.ismonster
			n["ischampion"] = e.ischampion
			n["isveteran"] = e.isveteran
			n["islegendary"] = e.islegendary
			n["iselite"] = e.iselite
			n["isheartquestgiver"] = e.isheartquestgiver
			table.insert(j,n)
			id,e = next(c,id)
		end
	else
