...
function gw2minion.ModuleInit()
	...
	local mw = WindowManager:NewWindow(gw2minion.MainWindow.Name,gw2minion.MainWindow.x,gw2minion.MainWindow.y,gw2minion.MainWindow.width,gw2minion.MainWindow.height)
	if ( mw ) then
	...
	if ( ml_blacklist_mgr ) then
		...
		if not ml_blacklist.BlacklistExists(GetString("sellItems")) then
			ml_blacklist.CreateBlacklist(GetString("sellItems"))
		end
		...
		if not ml_blacklist.BlacklistExists(GetString("vendorsrepair")) then
			ml_blacklist.CreateBlacklist(GetString("vendorsrepair"))
		end
