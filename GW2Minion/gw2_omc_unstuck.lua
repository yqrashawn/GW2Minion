...
function gw2_omc_unstuck.HandleStuck(type,startPos,endPos)
	...
	local dist3D = Distance3DT(ml_global_information.Player_Position,gw2_omc_unstuck.targetOMC.startPos)
	local dist2D = Distance2D(ml_global_information.Player_Position.x,ml_global_information.Player_Position.y,gw2_omc_unstuck.targetOMC.startPos.x,gw2_omc_unstuck.targetOMC.startPos.y)
	...
	if(distmoved > gw2_omc_unstuck.stuckDistance and ((ml_mesh_mgr.OMCStartTime > 0 and TimeSince(ml_mesh_mgr.OMCStartTime) < 2000) and not ml_mesh_mgr.OMCStartPositionReached) and gw2_omc_unstuck.targetOMC.stuckCount < gw2_omc_unstuck.STUCKSTATES.MinStuckCount) then
		gw2_omc_unstuck.lastResult = false
		return false
	end
	...
	if(gw2_omc_unstuck.targetOMC.stuckCount < gw2_omc_unstuck.STUCKSTATES.MinStuckCount and not ml_mesh_mgr.OMCStartPositionReached and ml_mesh_mgr.OMCStartTime > 0 and TimeSince(ml_mesh_mgr.OMCStartTime) > 3000) then
		gw2_omc_unstuck.Log("We seem to be stuck at the OMC start position.")
		gw2_omc_unstuck.targetOMC.stuckCount = gw2_omc_unstuck.STUCKSTATES.MinStuckCount+1
	end
	...
	if(gw2_omc_unstuck.targetOMC.stuckCount > gw2_omc_unstuck.STUCKSTATES.MinStuckCount) then
		...
		elseif(gw2_omc_unstuck.targetOMC.avoidanceAreaAdded == false and (gw2_omc_unstuck.targetOMC.stuckCount == gw2_omc_unstuck.STUCKSTATES.AddAvoidance or gw2_omc_unstuck.targetOMC.resetCount == 2)) then
			...
			gw2_obstacle_manager.AddAvoidanceArea({pos = gw2_omc_unstuck.targetOMC.startPos, radius=50 })
		...
		elseif(gw2_omc_unstuck.targetOMC.obstacleAdded == false and gw2_omc_unstuck.targetOMC.stuckCount == gw2_omc_unstuck.STUCKSTATES.AddObstacle) then
			gw2_omc_unstuck.StopMovement()
			gw2_obstacle_manager.AddObstacle({pos = gw2_omc_unstuck.targetOMC.startPos, radius=150 })
