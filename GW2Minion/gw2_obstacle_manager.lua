...
function gw2_obstacle_manager.AddObstacle(options)
	options = deepcopy(options and table_merge(ObstacleOptions,options) or ObstacleOptions)
	...
	if(ValidTable(target)) then
		...
		if(options.radius <= target.radius) then
			...
			for i=1,c do
				...
				a = a*(math.pi/180)
				local a2 = ((math.atan2(pos.hy,pos.hx)*(180/math.pi)) % 360)*(math.pi/180)
				if(target.isGadget) then
					a2 = (90*(math.pi/180))-a2
				end
...
function gw2_obstacle_manager.AddAvoidanceArea(options)
	options = deepcopy(options and table_merge(AvoidanceAreaOptions,options) or AvoidanceAreaOptions)
	if(ValidTable(options.pos)) then
		local newpos = NavigationManager:GetClosestPointOnMesh(options.pos)
		if(newpos ~= nil) then
			options.pos.z = newpos.z
...
function gw2_obstacle_manager.OnUpdateHandler(tick)
	if(TimeSince(gw2_obstacle_manager.ticks) > tonumber(gPulseTime or 150)) then
		for i,obstacle in pairs(gw2_obstacle_manager.obstacles) do
