				...
				if ( gamestate == GW2.GAMESTATE.GAMEPLAY ) then 
					...
					if ( GUI:TreeNode("MapMarker") ) then						
						...
						if ( GUI:TreeNode("Events") ) then
							if ( GUI:TreeNode("Nearest##event") ) then							
								local list = MapMarkerList("isevent,nearest")
								GUI:PushItemWidth(250)
								if ( table.valid(list) ) then local id,e = next(list) dev.DrawMapMarkerDetails(id,e) else	GUI:Text("No nearby event") end	
								GUI:PopItemWidth()
								GUI:TreePop()	
							end
...
function dev.DrawMapMarkerDetails(id,b)
	local charid = b.characterid > 0 and b.characterid or b.eventid > 0 and b.eventid or id
	
	local uniqid = (string.valid(b.name) and b.name or "unknown").."##"..tostring(charid)..tostring(b.contentid)..tostring(b.markertype)..tostring(b.worldmarkertype)
					...
					if ( GUI:TreeNode("ObjectiveDetails") ) then
						for eid,ee in pairs(eo) do
							if ( GUI:TreeNode("ID: "..tostring(eid) )) then
								...
								GUI:BulletText("Type") GUI:SameLine(200) GUI:InputText("##deveo1",tostring(ee.type))					
								GUI:TreePop()
							end	
						end
						GUI:TreePop()
